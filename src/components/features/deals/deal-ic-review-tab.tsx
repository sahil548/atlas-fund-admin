  ...
  async function handleICDecision(decision: string) {
    ...
    try {
      await fetch(`/api/deals/${deal.id}/ic-decision`, {
        ...
        body: JSON.stringify({ decision, userId: "user-jk" }),
      });
      toast.success(
    ...
    try {
      await fetch(
        `/api/deals/${deal.id}/ic-questions/${questionId}/replies`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ authorId: "user-jk", content }),
        }
      );
    ...
    <div className="space-y-4">
      {deal.icProcess ? (
        ...
        <div className="text-sm text-gray-500">
          No IC process initiated yet. Send the deal to IC Review from the Due
          Diligence stage.
        </div>
      )}
