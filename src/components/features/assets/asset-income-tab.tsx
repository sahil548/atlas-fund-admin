...
const CATEGORY_COLORS: Record<string, "blue" | "green" | "purple" | "orange" | "yellow" | "gray"> = {
  INTEREST: "blue",
  DIVIDEND: "green",
  RENTAL: "purple",
  ROYALTY: "orange",
...
export function AssetIncomeTab({
  ...
  const incomeEvents: any[] = data?.income ?? [];
  const totals = data?.totals ?? { totalIncome: 0 };

  // Group by category for subtotals
  const byCategory: Record<string, number> = {};
  for (const e of incomeEvents) {
    const cat = e.incomeType || "OTHER";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = typeof d.error === "string" ? d.error : "Failed to save income entry";
        setFormError(msg);
        return;
      }

      revalidate();
      setForm({ category: "RENTAL", amount: "", date: new Date().toISOString().slice(0, 10), description: "" });
      setShowForm(false);
    } finally {
      setSaving(false);
    }
  ...
  async function handleEditSave() {
    ...
    setEditSaving(true);
    try {
      const res = await fetch(`/api/assets/${assetId}/income/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          incomeType: editForm.category,
          amount,
          date: editForm.date,
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/assets/${assetId}/income/${deleteTarget.id}`, {
        method: "DELETE",
      });
      if (!res.ok) {
        const d = await res.json();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {incomeEvents.length} entr{incomeEvents.length !== 1 ? "ies" : "y"} recorded
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-[10px] text-gray-400 uppercase">Total Income</div>
              <div className="text-lg font-bold text-emerald-700">{fmt(totals.totalIncome)}</div>
            </div>
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="bg-gray-50 rounded-lg p-4 mb-4">
            <div className="text-xs font-semibold text-gray-700 mb-3">New Income Entry</div>
      ...
      {/* Transaction list */}
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <h3 className="text-sm font-semibold text-gray-900 mb-3">Income History</h3>
        {incomeEvents.length === 0 ? (
          <div className="text-center py-8">
            <div className="text-sm text-gray-400">No income recorded yet</div>
            <div className="text-xs text-gray-300 mt-1">
              Add income entries above to track rental, interest, dividends and more.
            </div>
          </div>
        ) : (
                  ...
                  editingId === e.id ? (
                    ...
                    <tr key={e.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="py-2.5 pr-4">
                        <Badge color={CATEGORY_COLORS[e.incomeType] ?? "gray"}>
                          {categoryLabel(e.incomeType)}
                        </Badge>
                      </td>
                      <td className={cn("py-2.5 pr-4 text-right font-semibold text-emerald-700")}>
                        +{fmt(e.amount)}
                      </td>
      ...
      <ConfirmDialog
        ...
        message={
          deleteTarget
            ? `Delete the ${categoryLabel(deleteTarget.incomeType)} entry for ${fmt(deleteTarget.amount)} on ${formatDate(deleteTarget.date)}? This cannot be undone.`
            : ""
        }
        confirmLabel="Delete"
        variant="danger"
