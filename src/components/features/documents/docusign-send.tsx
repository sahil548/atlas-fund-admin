...
/**
 * "Send for Signature" action for the Document Center.
 * Can be embedded in document row actions or inline cards.
 */
export function DocuSignSend({ documentId, documentName, dealId, entityId }: DocuSignSendProps) {
  ...
  // Fetch existing signature package for this document
  const { data: packages, mutate: mutatePackages } = useSWR(
    `/api/esignature?documentId=${documentId}`,
    fetcher
  );
  ...
  if (!isConnected) {
    return null; // Don't show the button if DocuSign not connected
  }
